# Called after routed OpenRCX SPEF has been written and read into final STA.
set spef $::env(RESULTS_DIR)/6_final.spef
if {![file exists $spef] || [file size $spef] == 0} {
    error "Full-PREP checkpoint requires nonempty routed SPEF"
}
report_units
set units_file [open $::env(REPORTS_DIR)/physical_units.rpt w]
puts $units_file "OpenROAD report_units is recorded in flow.log."
puts $units_file "Liberty units asserted by run_physical.sh: time=1ns capacitance=1fF."
close $units_file
...
report_checks -path_delay min -format full_clock_expanded \
    -group_path_count 10 -endpoint_path_count 1 -digits 4 \
...
close $f
set types [open $::env(REPORTS_DIR)/physical_cell_types.tsv w]
foreach name [lsort [array names master_counts]] {
    puts $types "$name\t$master_counts($name)"
}
close $types
puts "FULL_PREP_PHYSICAL_CHECKPOINT_COMPLETE SPEF=$spef DFFS=$flop_count"
